...
mergeSort:
	...
	cmp rsi, rdx		; rsi is left and rdx is right check if left >= right
	jge done			; jump to end
	xor r10, r10 		; reset rbx
	add	r10,rsi 		; add left to middle
	add	r10,rdx			; add right to middle
	shr r10, 1			; divide by 2 to get middle
	push rdx 			; store right
	push rsi 			; store left
	mov rdx, r10		; set right to middle
	push r10			; callee
	call mergeSort 	; mergeSort(array, left, middle)
	pop r10 			; retrieve middle
	pop rsi 			; retrieve left
	pop rdx 			; retrieve right
	push rsi 			; store left
	push rdx 			; store right
	mov rsi, r10		; set left to middle 
	inc rsi 			; increment to get (middle+1)
	push r10			; callee
	call mergeSort		; mergeSort(array, middle+1, right)
	pop r10 			; retrieve middle
	pop rdx 			; retrieve right
	pop rsi 			; retrieve left
	mov rcx, rdx		; set parameter 4 to be right
	mov rdx, r10		; set parametr 3 to be middle
	push rdx 			; push middle
	push r10 			; push 
	call merge 		; call merge
	pop r10 			; retrieve
	pop rdx 			; retrieve middle
done:
	ret 				; end
